...
class MaskedVisionTransformer(nn.Module):
    ...
    def forward(self, x, binary_map=None):
        # embed patches
        ...
        if binary_map is not None:
            kernel_size = [self.patch_embed.t_patch_size] + list(self.patch_embed.patch_size)
            pooled_map = F.avg_pool3d(binary_map,
                                      kernel_size=kernel_size,
                                      stride=kernel_size).flatten(3)
            pooled_map: torch.Tensor = torch.einsum("bcts->btsc", pooled_map).flatten(1,2)
            pooled_map = (pooled_map >= 0.2).type(torch.float32)
            
            x = (x[:, 1:, :] * pooled_map).mean(dim=1)
        else:
...
def masked_vit_base_patch16(**kwargs):
...
def masked_vit_large_patch16(**kwargs):
    model = MaskedVisionTransformer(
        patch_size=16,
        embed_dim=1024,
        depth=24,
...
def masked_vit_huge_patch14(**kwargs):
    model = MaskedVisionTransformer(
        patch_size=14,
        embed_dim=1280,
        depth=32,
