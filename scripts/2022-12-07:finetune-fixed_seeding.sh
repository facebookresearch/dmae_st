...
for TEMP in -1e10 -2e0 -1e0 -0.5e0 0; do
    python src/mae_submitit_finetune.py --job_dir="$OUT_DIR/DMAE_ST-full-TEMP$TEMP" \
        --num_gpus=8 --nodes=4 --exclude="" \
        --partition=learnai4p --account=all \
        --distributed \
        --epochs=100 --warmup_epochs=10 --dist_eval --checkpoint_period=10 \
        ...
        --cls_embed --sep_pos_embed --clip_grad=5.0 --fp32 \
        --finetune_fraction=0.1 \
        --finetune="/fsx/kart/skel_act_recg/experiments/dmae-st/DMAE_ST-full-sweep/DMAE_ST-full-TEMP$TEMP/checkpoints/checkpoint-00199.pth" # extra /checkpoints/ folder
done