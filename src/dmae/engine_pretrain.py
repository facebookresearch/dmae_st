...
def train_one_epoch(
    ...
    for data_iter_step, (samples, bbox_map, _) in enumerate(
        ...
        p_in, p_in_mask, p_out, p_out_mask = compute_masking_ratios(
            mask,
        ...
        if torch.isfinite(p_in_mask).all():  # may NaN when p_in=0 for entire batch
            metric_logger.update(inmsk=p_in_mask)
