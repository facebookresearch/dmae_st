...
from tsfmr.model.pose_classification_transformer import PoseClassificationTransformer
...
parser.add_argument("--hybrid_pos_embed", type=bool,
                    help="""Positional embedding hybrid between learnable and sinusoid.""")
...
def experiment(args):
    # Sync configs
    config = get_config(args.config.replace("default", "nturgbd_skeletons_pose_transformer"))
    ...
    # Model
    model = PoseClassificationTransformer(num_frames=example["x"].shape[0],
                                          num_skeletons=example["x"].shape[1],
                                          num_joints=example["x"].shape[2],
                                          ...
                                          hybrid_pos_embed=config.hybrid_pos_embed)
