...
def pyav_decode(
    ...
    if rigid_decode_all_video or duration is None:
...
def decode(
    ...
    assert rigid_decode_all_video or clip_idx >= -1, "Not valied clip_idx {}".format(clip_idx)
    try:
        if backend == "torchvision":
            ...
            # if max_spatial_scale is not 0:
            #     print("Max spatial scale not zero.")
            ...
            # pyav does not have built-in resizing, do manually with torchvision
            if max_spatial_scale != 0:
                frames = resize(
                    frames.permute(0, 3, 1, 2),
                    size=max_spatial_scale
                ).permute(0, 2, 3, 1)
