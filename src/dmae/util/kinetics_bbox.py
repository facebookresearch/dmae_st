    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self.num_retries):
            ...
            if (self.backend == "torchvision" and 
                "/".join(self._path_to_videos[index].split("/")[-2:]) in set([
            ...
            # Load bbox data, no try-catch as this should not fail.
            ...
            with open(self._path_to_bboxes[index], 'rb') as bbox_f:
                ...
                # for idx, bboxes in enumerate(bbox_list):
                #     bboxes = bboxes[:, :4] # drop probability value
                #     bbox_map[idx] = bbox_to_binary_map(bboxes, bbox_size)
