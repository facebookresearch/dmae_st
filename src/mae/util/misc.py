...
class MetricLogger(object):
    def __init__(self, delimiter="\t", default_print=False):
        self.meters = defaultdict(SmoothedValue)
        self.delimiter = delimiter
        
        if default_print:
            self.print=default_print_fn
        else:
            self.print=print
...
def init_distributed_mode(args):
    ...
    print(
        "| distributed init (rank {}): {}, gpu {} of {}".format(
            args.rank, args.dist_url, args.gpu, torch.cuda.device_count()
        ),
        # flush=True,
    )
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, attempts=3):
    # try multiple times in case of filesystem failures
    if args.checkpoint_dir:
        d = args.checkpoint_dir
    else:
        d = args.output_dir
    ...
    for i in range(attempts):
        try:
            if loss_scaler is not None:
                ...
                checkpoint_path = "{}/checkpoint-{:05d}.pth".format(d, epoch)
                to_save = {
            ...
            print(f"Saving checkpoint attempt: {i}   Error: {e}")
