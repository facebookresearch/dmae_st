...
from mae.util.decoder.decoder import get_start_end_idx, temporal_sampling
...
class Kinetics(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        csv_file_name = {
            ...
            "finetune": "train",
            "val": "val",
            "test": "test" if self.testset else "val", # Test time multi-view evaluation is done on validation data
        }
        ...
        folder = os.path.join(self._path_to_data_dir, csv_file_name[self.mode] + "_288px")
        if not os.path.isdir(folder):
            folder = os.path.join(self._path_to_data_dir, csv_file_name[self.mode])
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            # torchvision backend video_reader on cluster dies with a memory error when loading these examples.
            # So we use pyav!
            if "/".join(self._path_to_videos[index].split("/")[-1:]) in set([
                    # These are for CVDF
            ...
            try:
                frames, fps, decode_all_video = decoder.decode(
                    ...
                    num_clips=self._test_num_ensemble_views,
                ...
                if log_fail: 
                    print(
                        "Failed to DECODE video from {} with error{}".format(
                            self._path_to_videos[index], e
                        )
                    )
            ...
            if (frames is None) or (not torch.isfinite(frames).all()):
                if log_fail:
                    print(
                        "Decoded video idx {} from {}; trial {} empty or non-finite.".format(
